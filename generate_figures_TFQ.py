...
- TFQ_peerrank_correlation: Correlation scatter (peer scores vs ground truth accuracy)
- TFQ_score_comparison: Score comparison bar chart
- TFQ_rank_agreement: Rank agreement visualization
- TFQ_ablation_study: Ablation study (Peer vs Self correlation)
- Statistical analysis reports (TFQ_stats_report.txt, TFQ_stats_summary.json)
...
from peerrank.figure_utils import (
    ...
    get_text_color_for_background, get_comparison_data, get_correlation_stats,
    save_figure,
)
...
# are imported from peerrank.figure_utils (see import at top of file).
...
# get_comparison_data and get_correlation_stats are imported from peerrank.figure_utils.
...
# save_figure is imported from peerrank.figure_utils.
...
def generate_stats_report(data: dict, output_dir: Path):
    ...
    if correlation:
        ...
        report_lines.append("  for objective factual accuracy on TruthfulQA-style questions.")
    else:
