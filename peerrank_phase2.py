...
async def phase2_answer_questions() -> dict:
    ...
    costs = {n: {"total": 0.0, "llm_cost": 0.0, "tavily_cost": 0.0, "calls": 0, "input_tokens": 0, "output_tokens": 0} for _, _, n in MODELS}
    errors = []
    ...
    async def answer(provider, model_id, name, idx, text):
        try:
            ...
            ans, duration, input_tokens, output_tokens, tavily_cost = await call_llm(provider, model_id, prompt, max_tokens=MAX_TOKENS_ANSWER, use_web_search=web_search_enabled)
            llm_cost = calculate_cost(model_id, input_tokens, output_tokens)
            return (name, idx, ans, duration, input_tokens, output_tokens, llm_cost, tavily_cost, None)
        except Exception as e:
            print(f"    [ERROR] {name} Q#{idx}: {type(e).__name__}: {str(e)[:100]}", flush=True)
            return (name, idx, f"Error: {e}", 0, 0, 0, 0.0, 0.0, {"model": name, "q": idx, "error": str(e)[:200]})

    async def process_model(provider, model_id, name, semaphore):
        """Process all questions for a single model."""
        model_start = time.time()
        model_timing = []
        model_costs = {"total": 0.0, "llm_cost": 0.0, "tavily_cost": 0.0, "calls": 0, "input_tokens": 0, "output_tokens": 0}
        model_answers = {}
        model_errors = []
        ...
        for i in range(0, len(questions), 5):
            ...
            for _, idx, ans, duration, input_tokens, output_tokens, llm_cost, tavily_cost, err in results:
                ...
                model_answers[idx] = {
                    ...
                    "tavily_cost": round(tavily_cost, 6),
                    "cost": round(total_cost, 6)
                }
                model_timing.append(duration)
                model_costs["total"] += total_cost
                model_costs["llm_cost"] += llm_cost
                model_costs["tavily_cost"] += tavily_cost
                model_costs["calls"] += 1
        ...
        tavily_str = f" +${model_costs['tavily_cost']:.4f} Tavily" if model_costs['tavily_cost'] > 0 else ""
        print(f"  {name}: {len(questions)}/{len(questions)} (avg {avg_time:.2f}s/q, ${avg_cost:.4f}/q) | {format_duration(time.time() - model_start)} | ${model_costs['llm_cost']:.4f}{tavily_str}", flush=True)
    ...
    total_llm_cost = sum(c["llm_cost"] for c in costs.values())
    total_tavily_cost = sum(c["tavily_cost"] for c in costs.values())
    ...
    output = {
        ...
        "total_tavily_cost": round(total_tavily_cost, 4),
        "questions": questions,
        "errors": errors,
    ...
    tavily_str = f" (LLM: ${total_llm_cost:.4f} + Tavily: ${total_tavily_cost:.4f})" if total_tavily_cost > 0 else ""
    print(f"\n{'=' * 60}\nPhase 2 complete: {len(questions) * len(MODELS)} answers ({len(errors)} errors) in {format_duration(time.time() - phase_start)}\nTotal Cost: ${total_cost:.4f}{tavily_str}\n{'=' * 60}")
    return output
